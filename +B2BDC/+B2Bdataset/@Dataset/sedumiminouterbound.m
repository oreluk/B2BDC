...
minSensitivity.expu = zeros(n_units,1);
minSensitivity.expl = zeros(n_units,1);
for i1 = 1:n_units
   if isa(obj.DatasetUnits.Values(i1).SurrogateModel,'B2BDC.B2Bmodels.QModel')
      minSensitivity.expu(i1) = -lamEU(i1)*d(i1)*S(1,1);
      minSensitivity.expl(i1) = -lamEL(i1)*d(i1)*S(1,1);
   elseif isa(obj.DatasetUnits.Values(i1).SurrogateModel,'B2BDC.B2Bmodels.RQModel')
      ...
      minSensitivity.expu(i1) = -lamEU(i1)*d(i1)*trace(tmpD*S);
      minSensitivity.expl(i1) = -lamEL(i1)*d(i1)*trace(tmpD*S);
   end
end
minSensitivity.varu = -1.05*lamVU.*vd.^2;
minSensitivity.varl = -1.05*lamVL.*vd.^2;
if n_extra ~= 0
   count = 1;
   for k1 = 1:n_extra
      i1 = extraIdx(k1,1);
      ...
      minSensitivity.varu(i1) = minSensitivity.varu(i1)+vd(i1)*(tmpLam(3)*(vl(j1)*S(1,1)-S(1,j1))+tmpLam(4)*(-vu(j1)*S(1,1)+S(1,j1)));
      minSensitivity.varu(j1) = minSensitivity.varu(j1)+vd(j1)*(tmpLam(2)*(vl(i1)*S(1,1)-S(1,i1))+tmpLam(4)*(-vu(i1)*S(1,1)+S(1,i1)));
      minSensitivity.varl(i1) = minSensitivity.varl(i1)+vd(i1)*(tmpLam(1)*(vl(j1)*S(1,1)-S(1,j1))+tmpLam(2)*(-vu(j1)*S(1,1)+S(1,j1)));
      minSensitivity.varl(j1) = minSensitivity.varl(j1)+vd(j1)*(tmpLam(1)*(vl(i1)*S(1,1)-S(1,i1))+tmpLam(3)*(-vu(i1)*S(1,1)+S(1,i1)));
      count = count+4;
   end
end
