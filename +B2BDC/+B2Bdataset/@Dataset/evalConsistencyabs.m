...
% absolute value) of a B2BDC.B2Bdataset.Dataset object
...
%    [yout,sensitivity] = obj.cvxconsisquadabs(frac);
   ...
   if n_outer > 1
      ...
      for j = 1:n_outer
         [yout_result(j),sensitivity{j}] = obj.sedumiconsisabs(yin,frac,tol);
         if disflag
...
%       [yout_result(1),sensitivity{1}] = obj.cvxconsisabs(yin,frac,tol);
   end
   ...
   function [c,ceq,g,geq] = neq(x)
      ...
      for i = 1:n_units
         ...
         if isa(model,'B2BDC.B2Bmodels.RQModel')
            ...
            c(2*i-1,1) = ([1;x(id1)]'*N*[1;x(id1)])/([1;x(id1)]'*D*[1;x(id1)])-u+x(1);
            c(2*i,1) =  l+x(1)-([1;x(id1)]'*N*[1;x(id1)])/([1;x(id1)]'*D*[1;x(id1)]);
         ...
         elseif isa(model,'B2BDC.B2Bmodels.QModel')
            ...
            c(2*i-1,1) = [1;x(id1)]'*quadCoef*[1;x(id1)]-u+x(1);
            c(2*i,1) =  l+x(1)-[1;x(id1)]'*quadCoef*[1;x(id1)];
            grad1 = zeros(n_variable+1,1);
            grad2 = zeros(n_variable+1,1);
